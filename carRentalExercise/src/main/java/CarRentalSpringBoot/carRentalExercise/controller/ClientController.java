...
public class ClientController {
    ...
    @PostMapping("/")
    public ResponseEntity<ClientCreateDto> addNewClient(@Valid @RequestBody ClientCreateDto client) throws ClientAlreadyExists {
        // clientService.addNewClient(client);
        return new ResponseEntity<>(clientService.addNewClient(client), HttpStatus.CREATED);
    }

    @PatchMapping(path = "{clientId}")
    public ResponseEntity<ClientGetDto> updateClient(@PathVariable("clientId") Long id, @Valid @RequestBody ClientPatchDto client) throws ClientIdNotFoundException {
